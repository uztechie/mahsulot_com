...
@AndroidEntryPoint
class MarjaProductDetailsFragment:Fragment(R.layout.fragment_product_details) {
    var product = Product()
    var totalPrice = 0
    var amount = 0
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        product = arguments?.let { MarjaProductDetailsFragmentArgs.fromBundle(it).product }!!
        ...
        details_price.text = "${getString(R.string.marja)}: ${product.marja_price}"
        ...
        details_buy_btn.setOnClickListener {
            viewModel.getUser().observe(viewLifecycleOwner, Observer {
                if (it == null){
                    findNavController().navigate(MarjaProductDetailsFragmentDirections.actionGlobalLoginFragment())
                }
                else{
                    amount = elegantNumber.number.toInt()
                    findNavController()
                        .navigate(MarjaProductDetailsFragmentDirections.actionMarjaProductDetailsFragmentToCreateMarjaOrderFragment(
                            product = product
                        ))
                }
            })
    ...
    fun calculatePrice(quantity:Int){
        product.marja_price?.let {
            totalPrice = quantity*it
            details_buy_btn.text = "${getString(R.string.harid_qilish)}  (${getString(R.string.marja)}: ${totalPrice})"
        }
